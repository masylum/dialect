...
module.exports = function (options) {
        ...
        options.locales.forEach(function (locale) {
          ...
          if (options.store) {
            options.store.add({original: original, locale: locale}, translation, fk.add(function (err, data) {
              loadDictionariesToJSON(true, fk.add(function () {}));
            }));
          } else {
            addTranslationToJSON({original: original, locale: locale}, translation, fk.add(function () {}));
          }
        });
  ...
  dialect.setTranslation = function (query, translation, callback) {
    ...
    // DB
    if (options.store) {
      options.store.set(query, translation, fk.add(function (err, data) {})); // fire and forget
    }

    // JSON store
    addTranslationToJSON(query, translation, fk.add(function () {}));
